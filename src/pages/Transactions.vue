...
let transactionNote = ref("")
let transactionBalance = ref(0)
...
let transactionsColl = null;
let categoriesColl = null;
let feedBack = ref(null);


const loadData = async () => {
    walletsColl = await useCollection("wallets");
    transactionsColl = await useCollection("transactions");
    categoriesColl = await useCollection("categories");

    wallets.value = walletsColl.myCollection;
    transactions.value = transactionsColl.myCollection;
    categories.value = categoriesColl.myCollection;
}
...
const addTransactionDoc = async () => {
    console.log(transactionName.value, transactionNote.value, transactionBalance.value);
    transactionsColl.addDocumentData("transactions", {
        name: transactionName.value,
        note: transactionNote.value,
        balance: transactionBalance.value,
    ...
    transactionBalance.value = 0;
    closeModal();
}
const checkTransactionBalance = () => {
    if (transactionBalance.value > selectedWallet.value.wBalance) {
        feedBack.value = "Transaction's balance value must be less than your wallet's balance value!"
    }
}
...
<template>
    <SimpleModal :is-open="isOpenModal" :modal-title="'Transaction'">
        <form @submit.prevent="addTransactionDoc" method="post">
            ...
            <div class="mb-4 w-full flex justify-start items-center">
                <select id="cw" name="currentWallet" v-model="selectedCategories"
                    class="w-full p-3 mt-4 rounded-md text-gray-800">
                    <option :disabled="true" :selected="true">--- Choose your wallet --</option>
                    <option v-for="category in categories" :key="category.id" :value="category">{{ category.categoryName }}</option>
                </select>
            </div>
            <div class="mb-4 w-full flex justify-start items-center">
                <input :disabled="selectedWallet === ''" v-model="transactionName" placeholder="Transaction Name..."
                    class="w-full mr-3 p-2 bg-gray-50 disabled:cursor-not-allowed" type="text" name="transactionName" id="transaction-name">
            </div>
            <div class="mb-4 w-full flex justify-start items-center">
                <input :disabled="selectedWallet === ''" v-model="transactionNote" placeholder="Note..."
                    class="w-full mr-3 p-2 bg-gray-50 disabled:cursor-not-allowed" type="text" name="transactionNote" id="transaction-note">
            </div>
            <div class="mb-4 w-full flex flex-wrap justify-start items-center">
                <input @change="checkTransactionBalance" :disabled="selectedWallet === ''"
                    v-model.number="transactionBalance" placeholder="$" class="w-full mr-3 p-2 bg-gray-50 disabled:cursor-not-allowed" type="text"
                    name="transactionBalance" id="transaction-balance">
        ...
        <div class="flex flex-row-reverse">
            <button class="bg-red-200 text-red-600 hover:text-white hover:bg-red-600 hover:shadow-md hover:shadow-red-900 p-3 rounded-lg transition-all duration-150" @click="closeModal">Cancel</button>
        </div>
    </SimpleModal>
    <!-- create transactions form -->
    <div class="w-1/3 bg-white m-0 mx-auto p-5 mb-4">
        <button class="rounded-full p-2 bg-gray-300 hover:bg-white hover:shadow-lg" @click="openModal" type="button">Add a transaction</button>
